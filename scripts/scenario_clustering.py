"""
scenario_clustering.py — Layer 2 of TencentDB Agent Memory 4-tier architecture.

What it does:
- Takes atomic facts from bus_discrete (Layer 1) + memu persona items
- Groups them by project/scenario using embedding similarity + keyword clustering
- Produces a "scenario graph" stored in mempalace: scenarios[] → facts[] with TTL/decay per scenario
- On cold-start, query top-N scenarios by (recency × importance) instead of dumping all facts

Architecture mapping:
- Layer 0 (raw) = bus_discrete per-turn decisions
- Layer 1 (facts) = bus_discrete fact records + memu items
- Layer 2 (scenarios) = THIS MODULE → produces scenario clusters
- Layer 3 (profile) = memu profile (already in place)
...
BUS_DB = r"D:\AI\memory-system\memory-bus\memory_bus.db"
MEMU_URL = "http://127.0.0.1:7801"
MEMPALACE_URL = "http://127.0.0.1:7802"
SCENARIO_STORE = Path(r"D:\AI\memory-system\memory-bus\scenarios.db")
...
def init_scenario_db():
    ...
    conn.executescript("""
        CREATE TABLE IF NOT EXISTS scenarios (
            ...
            fact_ids TEXT,            -- JSON array of bus_discrete IDs
            importance REAL DEFAULT 0.5,   -- 0-1, decay slower when high
            created_at REAL NOT NULL,
            updated_at REAL NOT NULL,
            last_accessed REAL,
            access_count INTEGER DEFAULT 0,
            ttl_days INTEGER DEFAULT 30    -- scenario-level TTL (vs per-fact)
        );
...
def fetch_recent_facts(since_days: int = 7):
    """Layer 1 → pull canonical facts from memory_canonical table.
    Schema (verified 2026-07-31): id, kind, content, tags, importance,
    promoted_at, last_accessed_at, access_count, scene, stable_id.
    """
    ...
    rows = conn.execute(
        """SELECT id, kind, content, tags, importance, promoted_at, scene, stable_id
    ...
    for r in rows:
        ...
        facts.append({
            "id": str(r[0]),
            "kind": r[1],
            "summary": (r[2] or "")[:200],   # first 200 chars as summary
            "content": r[2] or "",
            "tags": tags,
...
def hydrate(query: str, top: int = 3):
    ...
    Falls back to current memory_canonical fact summaries (no full text body).
    """
    init_scenario_db()
    conn = sqlite3.connect(SCENARIO_STORE)
    rows = conn.execute(
    ...
    for row in rows:
        ...
        # Decay by days since updated (updated_at is ISO string)
        try:
    ...
    # Fetch fact details from memory_canonical — dedupe fids first
    fact_details = {}
    if top_n:
        seen_ids = set()
        fact_ids = []
