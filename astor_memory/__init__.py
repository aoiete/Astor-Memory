...
__version__ = "1.15.0"  # 2026-09-22 (Ship v1.15.0 — time-scoped recall + hit-source provenance. NEW: nest/time_phrase.py parses Chinese+English natural time phrases (昨天/上周二/最近N天/last week/N days ago/ISO dates) out of /v1/read query text and auto-derives since_ts/until_ts when caller didn't pass them (Lossless-memory lesson: time is the primary axis — restrict range first, rank within it). Honest fallback: auto-scope yielding < min(3, top_k) results rolls back and reports time_fell_back=true. Response gains time_scoped {since, until, phrase} + time_fell_back. NEW: per-result hit_source field (bm25 / vector / bm25+vector / grep_verify / session_neighbor) so eval and callers can see which retrieval path found each fact. 6 new test groups in tests/test_time_phrase.py. 466 passed, 2 pre-existing failures unchanged (test_acl_bob_can_read_own_private, test_rest_read_returns_entities_field).)
