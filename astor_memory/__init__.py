...
__version__ = "1.15.7"  # 2026-09-22 (Ship v1.15.7 — RRSI pass b: noise-floor gate on decay-sweep. NEW CLI flag --require-no-recall-days N (default 30, 0=legacy off) on both `am decay-sweep run` and `am decay-sweep stats`. Inspired by RRSI paper (arxiv 2609.24972) "noise floor" constraint: a small score delta isn't meaningful; a fact touched at least once in the window is too useful to be noise. Now even low-importance + low-read facts are PROTECTED if anyone recalled them in the last 30 days. Verified end-to-end on real runtime bus: tier=public max_imp=1.0 max_access=5 → 310 eligible (off) vs 288 (1d) vs 0 (30d). The 30d window protects 310 facts that have been recalled in the last month; the 1d window protects 22 that have been recalled in the last day. No DB migration, no public API change (flag has default; legacy 0=off behavior preserved). 468 tests pass.)
