...
__version__ = "1.14.71"  # 2026-09-17 (Ship v1.14.71 — S1 multi-topic extension. Server /v1/read now accepts `topics=[...]` array in addition to single `topic=` (backwards compat). When multiple topics match the same fact's tags, boosts compose (factor^matches — e.g. 2 matching topics with factor=1.10 → 1.10^2 = 1.21 boost). Three input formats supported: `topic=X` (str), `topics=["X","Y"]` (list), `topics_str="X,Y"` (CSV). Response adds `topics_used` field echoing the deduped topic set. CLI `am peer topic set` extended: comma-separated topics + matching `--weights=0.9,0.5,0.3` (or single `--weight` broadcast across all topics). 2 new tests added (composing boost + multi-input parsing) for 15/15 in test_topic_routing.py. 103/103 in ship subset. Use case: recall with `topics=["poker","nlhe"]` boosts a fact tagged both higher than one tagged only one, breaking semantic ties by topic relevance.)
