...
__version__ = "1.14.70"  # 2026-09-17 (Ship v1.14.70 — S1 topic-aware routing. NEW: topic_index table in $ASTOR_DIR/identity/relationships.db (cols: topic, peer_id, weight, fact_count, last_seen_at, source). CRUD: set_topic/get_topic/list_topics_for_peer/list_peers_for_topic/list_all_topics/bump_topic_seen/remove_topic. NEW CLI under `am peer topic`: set|list|list-peers|remove|bump (5 commands). Server: /v1/read body now accepts `topic=X` parameter — boost facts whose tags contain X by topic_boost_factor (default 1.10x). Response includes `topic_boost_applied` flag. Soft routing: per-peer per-topic weight (0.0-1.0) lets admin express 'alice is great for poker (0.95) but only decent for fortune (0.40)'. 13 new tests in tests/test_topic_routing.py (CRUD + routing queries + boost logic unit test). Topic boost is conservative (1.10x) to avoid distorting semantic ranking — breaks ties, doesn't override. 168/168 tests in ship subset (Phase 1+2 + S1 combined). Closes S1 from v1.14.65 backlog.)
