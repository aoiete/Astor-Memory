...
__version__ = "1.15.2"  # 2026-09-22 (Ship v1.15.2 — default zone + recall-auto. (a) cmd_recall --zone default flipped from None (raw fact rows) to 'success' (highest-signal zone) so the most common agent query — "I want a proven recipe for X" — lands in success_pattern without forcing the caller to remember the flag. Explicit --zone=failure / lesson / all-zones / none (raw) still supported. (b) NEW CLI: `am recall-auto "<error>"` walks failure -> lesson -> success -> all-zones with the most informative tokens from the input (regex extracts alphanumeric runs >=2 chars, drops file paths, hex, common stopwords, dedupes), so the agent can paste an error message or pipe a log via stdin and get the right zone + top-3 facts without guessing. ZONE_KINDS promoted from argparse-local to module-level constant so cmd_recall_auto can reuse it. test_e2e_integration explicitly opts into --zone=none to keep the raw recall path covered. 468 tests pass 3+ rounds stable.)
