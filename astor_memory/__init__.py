...
__version__ = "1.15.1"  # 2026-09-22 (Ship v1.15.1 — Zone shortcuts for outcome-prioritized recall. NEW CLI flag: `am recall --zone {failure|success|lesson|all-zones}` as a one-word shortcut for `--kinds <zone-kind-list>`. Three named zones cover the common agent loop (failure: I just hit a wall, success: I want a proven recipe, lesson: postmortem/root-cause). `--kinds` always wins when both are passed (escape hatch). README + README.zh-CN.md: new "Zone shortcuts" + "Recall discipline for agents" sections. NEW scripts/smoke_recall_zone.sh — 5s liveness gate (compile + static grep + argparse --help verification). 466 tests unchanged from v1.15.0.)
